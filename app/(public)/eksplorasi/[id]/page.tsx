  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <section className="relative h-[340px] text-white">
        <DestImage src={dest.imageUrl} alt={dest.nama} kategori={dest.kategori} className="absolute inset-0 h-full w-full" />
        <div className="absolute inset-0 bg-[#0194F3]/10" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-transparent" />
        <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-8">
          <Link href="/eksplorasi" className="mb-3 w-fit text-sm font-semibold text-white/90 drop-shadow hover:underline">← Kembali ke Eksplorasi</Link>
          ...
          <h1 className="text-3xl font-extrabold drop-shadow-lg md:text-4xl">{dest.nama}</h1>
          <p className="mt-1 text-blue-50 drop-shadow">📍 {dest.wilayah} • {dest.jarakPusat} km dari pusat • ⭐ {dest.rating}</p>
        </div>
      </section>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-8 md:grid-cols-[1fr_340px]">
        ...
        <div className="space-y-6">
          ...
          {/* Spek logistik */}
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-xl font-bold text-slate-800">Spesifikasi Logistik & Akses</h2>
